...
/**
 * 类名称：RestfullController 提供微信端网页访问的接口 创建人：sys 更新时间：2016年4月21日
 * 
 * @version 1.0
 */
...
public class RestfullController extends BaseController {
	 ...
	 * 
	 * @param userID
	 * @return pd{ ISMODIFY 值为1时进行过修改， 值为0时未修改过 }
	 * @throws Exception
	 */
	...
	/**
	 * 用户注册，通过手机号码 url : http://localhost:8080/ihealth/rest/register type:post
	 * 
	 * @param {phone:"xx",openId:"xxx","name":"姓名","avatar":"用户头像"}
	 * @return 当手号码不存在时，执行新增用户并返回用户信息: {"result": "success","data":
	 *         {"OPENID":"OPENID","PHONE": "电话","USER_ID": "USER_ID","ROLE_ID":
	 *         "用户权限"}} 当用户执行添加关心的人的操作时，关系存在返回的数据 {"result":
	 *         "connection_existence"} 当手机号码存在，返回 { "result": "existence",
	 *         "data": { "NUMBER": "编号", "RIGHTS": "权限", "IP": "IP地址", "PHONE":
	 *         "电话", "ALIAS": "昵称", "SEX": "性别", "USER_ID": "ID",
	 *         "MARRIAGESTATUS": "婚姻状况", "LAST_LOGIN": "最后登录时间", "EMAIL":
	 *         "邮箱地址", "HEIGHT": 身高, "BIRTHPLACE": "出生地", "NAME": "姓名",
	 *         "CAREER": "职业", "STATUS": "0", "OPENID": "openId", "PASSWORD":
	 *         "密码", "BZ": "333", "USERNAME":"用户名", "DEGREE": "学历", "LIVEPLACE":
	 *         "常住地", "AVATAR": "头像地址", "WEIGHT": 体重, "BIRTHDAY": "生日" } }
	 */
	...
	private Object register(@RequestBody(required = true) String userVO) throws Exception {
		...
		try {
			 ...
			 * !"".equals(jasonObject.get("phone")) &&
			 * !"null".equals(jasonObject.get("phone"))) { phone =
			 * jasonObject.getString("phone").trim(); pd.put("PHONE", phone);
			 * pd.put("USERNAME", phone); }
			 */
			if (jasonObject.get("openId") != null && !"".equals(jasonObject.get("openId"))
					&& !"null".equals(jasonObject.get("openId"))) {
				openId = jasonObject.getString("openId").trim();
				pd.put("OPENID", openId);
			}
			if (jasonObject.get("avatar") != null && !"".equals(jasonObject.get("avatar"))
					&& !"null".equals(jasonObject.get("avatar"))) {
			...
			// 判断手机号码是否存在 如果当前手机号存在 则继续判断openID是否存在，如果都存在 则返回用户数据 如果不存在 则
			// 添加openID和用户头像信息
	 ...
	 * @return 手机号码为空，返回 {"result":"no"} 关系已存在，返回 {"result":"existence"}
	 *         关系不存在，返回 { "result":"success" "data": { "NUMBER": "编号", "RIGHTS":
	 *         "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称", "SEX": "性别",
	 *         "USER_ID": "ID", "MARRIAGESTATUS": "婚姻状况", "LAST_LOGIN":
	 *         "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高, "BIRTHPLACE": "出生地",
	 *         "NAME": "姓名", "CAREER": "职业", "STATUS": "0", "OPENID": "openId",
	 *         "PASSWORD": "密码", "BZ": "333", "USERNAME":"用户名", "DEGREE": "学历",
	 *         "LIVEPLACE": "常住地", "AVATAR": "头像地址", "WEIGHT": 体重, "BIRTHDAY":
	 *         "生日" } }
	 * 
	 * @throws Exception
	 */
	...
	public Object saveByPhone(@RequestBody(required = true) String message) throws Exception {
		...
		if (jasonObject.get("phone") == null || "" == jasonObject.get("phone") || jasonObject.get("userId") == null
				|| "" == jasonObject.get("userId")) {
			msg = "no";
		} else {
	...
	public Object addRepeatUser(@RequestBody String user) throws Exception {
		...
		if (msg.equals("success")) {
			PageData p = this.appuserService.findConnectionWhether(pd);
			if(p==null){
				pd.put("useranduser_id", this.get32UUID());
				pd.put("isprivacy", 0);
				pd.put("ismodify", 0);
	 ...
	 * 
	 * @param message
	 *            {"phone":"手机号码","userId":"当前用户ID"}
	 * @return 当通过手机号查询的数据为null时 返回{"result":"no"} 查询出有关系已存在时 返回
	 *         {"result":"guanxicunzai"} 查询关系不存在时 返回{"result":"guanxibucunzai"}
	 * @throws Exception
	 */
	...
	public Object getConnectionByPhone(@RequestBody(required = true) String message) throws Exception {
		...
		try {
			if (jasonObject.get("phone") != null && !"".equals(jasonObject.get("phone"))
					&& !"null".equals(jasonObject.get("phone"))) {
	...
	/**
	 * 根据ID更新用户信息 URL：http://localhost:8080/ihealth/rest/updateUser type:post
	 * 
	 * @return ID不为空 返回结果，和用户信息： { "result":"suceess", "data": { "NUMBER": "编号",
	 *         "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称",
	 *         "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS": "婚姻状况",
	 *         "LAST_LOGIN": "最后登录时间","EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业", "STATUS": "0",
	 *         "OPENID": "openId", "PASSWORD": "密码", "BZ": "333", "USERNAME":
	 *         "用户名", "DEGREE": "学历", "LIVEPLACE": "常住地", "AVATAR": "头像地址",
	 *         "WEIGHT": 体重, "BIRTHDAY": "生日" } } ID为空： {"result":"error"}
	 * @param {
	 *            "userId":"ID", "name":"姓名", "sex":"性别", "marriageStatus":"婚姻",
	 *            "birthday":"生日", "height":"身高", "weight":"体重",
	 *            "birthPlace":"出生地", "livePlace":"常住地", "career":"职业",
		...
		// 将String类型的数据转换为json
			...
			if (jasonObject.get("userId") != null && !"".equals(jasonObject.get("userId"))
				...
				if (jasonObject.get("marriageStatus") != null && !"".equals(jasonObject.get("marriageStatus"))
						&& !"null".equals(jasonObject.get("marriageStatus"))) {
				...
				if (jasonObject.get("weight") != null && !"".equals(jasonObject.get("weight"))
						&& !"null".equals(jasonObject.get("weight"))) {
				...
				if (jasonObject.get("birthPlace") != null && !"".equals(jasonObject.get("birthPlace"))
						&& !"null".equals(jasonObject.get("birthPlace"))) {
				...
				if (jasonObject.get("livePlace") != null && !"".equals(jasonObject.get("livePlace"))
						&& !"null".equals(jasonObject.get("livePlace"))) {
				...
				if (jasonObject.get("career") != null && !"".equals(jasonObject.get("career"))
						&& !"null".equals(jasonObject.get("career"))) {
				...
				if (jasonObject.get("degree") != null && !"".equals(jasonObject.get("degree"))
						&& !"null".equals(jasonObject.get("degree"))) {
	...
	/**
	 * 通过用户ID获取体检套餐信息 url:http://localhost:8080/ihealth/rest/findCheckPackage
	 * type:post
	 * 
	 * @param {"userId":"用户ID"}
	 * @return 当用户ID不为空时，返回： { "result": "success", "data": { "REVISION":
	 *         "版本","STATUS": "状态 pending / ready", "GENERATEDTIME": 该记录生成时间,
	 *         "EFFECTIVEFROM":开始生效时间, "EXPIREON": 失效时间, "WORKER": "生成该记录的标记",
	 *         "CHECKUPPACKAGE_ID": "1001", "SYSFLAG": "系统标记" } }
	 * 
	 *         当用户ID为空时，返回：{ "result": "error"} 当根据userID查询出的数据为null时
	 *         返回{"result": "no"}
	 * @throws Exception
	 */
	...
	public Object findCheckPackage(@RequestBody String u) throws Exception {
		...
		try {
			if (jasonObject.get("userId") != null && !"".equals(jasonObject.get("userId"))
				...
				if (data == null) {// return waiting status if checkuppackage is
									// not exists
					msg = "no";
				} else {
	...
	/**
	 * 根据体检项目ID修改体检项目的状态信息 url:http://localhost:8080/ihealth/rest/editCheckItem
	 * type:post
	 * 
	 * @param {"checkupItemId":"体检项目ID","stauts":"状态信息","subGroup":"分组名"}
	 * 
	 * @return 修改成功、返回 { "result": "success"} 修改失败、返回{"result": "error"}
	 * 
	 * @throws Exception
	 */
	...
	public Object editCheckItem(@RequestBody String check) throws Exception {
		...
		try {
			// if (jasonObject.get("subGroup") != null &&
			// !"".equals(jasonObject.get("subGroup")) &&
			// !"null".equals(jasonObject.get("subGroup"))) {
			...
			if (jasonObject.get("stauts") != null && !"".equals(jasonObject.get("stauts"))
					&& !"null".equals(jasonObject.get("stauts"))) {
				status = jasonObject.getString("stauts").trim(); // 获取状态
				// 如果当前状态为
				// 已选中 则将当前状态修改为 已删除
				if (status.equals(StatusEnum.ALREADYENABLED.getName())) {
					if (jasonObject.get("checkupItemId") != null && !"".equals(jasonObject.get("checkupItemId"))
							&& !"null".equals(jasonObject.get("checkupItemId"))) {
	...
	/**
	 * 根据userID 获取体检项目 url：http://localhost:8080/ihealth/rest/findCheckItems
	 * type:post
	 * 
	 * @param {"userId":"用户ID"}
	...
	public Object findCheckItems(@RequestBody String u) throws Exception {
		...
		try {
			if (jasonObject.get("userId") != null && !"".equals(jasonObject.get("userId"))
				...
				// List<PageData> pageDate =
				// this.checkupitemService.listCheckupItemSummary(pd);
	...
	/**
	 * 根据体检项目ID 获取体检项目 url：http://localhost:8080/ihealth/rest/getCheckItem
	 * type:post
	 * 
	 * @param {"id":"体检项目ID"}
	 * @return 根据体检项目ID，查询当前体检项目 { "result": "success", "data": [ { "REVISION":
	 *         版本, "STATUS":"状态，包括：已选中，已删除", "DESCRIPTION": "详细描述",
	 *         "GENERATEDTIME": 该记录生成时间, "FREQUENCY": "每年一次", "ORIGINATE":
	 *         "指南来源", "WORKER": "用于产生该记录的标记", "SUBGROUP": "检查项目分组", "SYSFLAG":
	 *         "系统标记", "NAME": "检查项目名称", "FEATURES": "检查频率，是文字描述",
	 *         "CHECKUPITEM_ID": "ID" } ] } 当用户ID为空时返回：{"result": "error"}
	...
	public Object getCheckItem(@RequestBody String u) throws Exception {
		...
		JSONObject jasonObject = JSONObject.fromObject(u);
		try {
			if (jasonObject.get("id") != null && !"".equals(jasonObject.get("id"))
					&& !"null".equals(jasonObject.get("id"))) {
	...
	/**
	 * 通过用户ID获取用户信息 url:http://localhost:8080/ihealth/rest/findUserById
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 * @return 当ID不为空，返回的参数列表为： { "result": "success", "data": { "NUMBER": "编号",
	 *         "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称",
	 *         "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS": "婚姻状况",
	 *         "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业",
	 *         "STATUS":"用户状态", "OPENID": "openId", "PASSWORD": "密码", "BZ":
	 *         "333", "USERNAME": "用户名", "DEGREE": "学历", "LIVEPLACE": "常住地",
	 *         "AVATAR":"头像地址", "WEIGHT": 体重, "BIRTHDAY": "生日" } }
	 *         当userID为空时：返回的参数为：{"result": "error"} 当根据userID查询出的数据为null时
	 *         返回{"result": "no"}
	 * @throws Exception
	 */
	...
	/**
	 * 根据openId获取用户信息 url:http://localhost:8080/ihealth/rest/findUserByOpenId
	 * type:post
	 * 
	 * @param {openid:"openId"}
	 * 
	 * @return 当获取openId成功！返回值为： { "result": "success", "data": { "NUMBER":"编号",
	 *         "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称",
	 *         "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS": "婚姻状况",
	 *         "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业",
	...
	/**
	 * 获取前20篇文章信息 url:http://localhost:8080/ihealth/rest/findByTopTwenty
	 * type:post
	 * 
	 * @return 获取文章信息不为空： { "result": "success", "data": [ { "LOGOURL": 照片url,
	 *         "PUBLISHTIME": 发布时间 , "SUMMARY": "摘要", "CREATEBY": "创建记录员工id",
	 *         "TITLE": "标题", "ARTICLE_ID": "ID", "AUTHOR": "作者",
	 *         "CREATEON":创建记录时间 , "URL": 内容url } ] }
	 * 
	...
	/**
	 * 获取所有文章信息 url:http://localhost:8080/ihealth/rest/findAllArticles type:post
	 * 
	 * @return 获取文章信息不为空： { "result": "success", "data": [ { "LOGOURL": 照片url,
	 *         "PUBLISHTIME": 发布时间 , "SUMMARY": "摘要", "CREATEBY": "创建记录员工id",
	 *         "TITLE": "标题", "ARTICLE_ID": "ID", "AUTHOR": "作者",
	...
	/**
	 * 根据文章信息ID 获取文章信息 url:http://localhost:8080/ihealth/rest/findArticleById
	 * type:post
	 * 
	 * @param {"articleId":"文章信息ID"}
	 * @return 根据ID查询出数据返回： { "result": "success", "data": { "LOGOURL": 照片url,
	 *         "PUBLISHTIME": 发布时间 , "SUMMARY": "摘要", "CREATEBY": "创建记录员工id",
	 *         "TITLE": "标题", "ARTICLE_ID": "ID", "AUTHOR": "作者",
	 *         "CREATEON":创建记录时间 , "URL": 内容url } } 当articleId为空： 返回{ "result":
	 *         "error"}
	 * @throws Exception
	 */
	...
	public Object findArticleById(@RequestBody String a) throws Exception {
		...
		if (jasonObject.get("articleId") != null && !"".equals(jasonObject.get("articleId"))
				&& !"null".equals(jasonObject.get("articleId"))) {
	...
	/**
	 * 通过用户ID 获取关联的用户信息 url:http://localhost:8080/ihealth/rest/findUsersById
	 * type:post
	 * 
	 * @param {"userId":"用户ID"}
	 * @return 当查询出数据时、返回一条或多条数据，取值需要循环： { "result": "success", "data": [ {
	 *         "useranduser_id": "关联表主键","connection": "我们之间的关系",
	 *         "useranduser_id": "我是谁", "user_id_two": "跟谁关联", "NUMBER": "编号",
	 *         "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称",
	 *         "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS":"婚姻状况",
	 *         "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业", "STATUS":
	 *         "用户状态", "OPENID": "openId", "PASSWORD": "密码", "BZ": "333",
	 *         "USERNAME": "用户名", "DEGREE": "学历", "LIVEPLACE": "常住地", "AVATAR":
	 *         "头像地址", "WEIGHT": 体重, "BIRTHDAY": "生日" } ] } 当传入的userId为null或为''时
	 *         返回数据为{ "result": "error"} 当根据userID查询出的数据为null时 返回{ "result":
	 *         "no"}
	 * @throws Exception
	 */
	 ...
	 * url:http://localhost:8080/ihealth/rest/deleteRelationUser type:post
	 * 
	 * @param {"useranduser_id":"值为：根据前面根据userId查询出来的useranduser_id"}
	 * @return 删除成功返回:{"result": "success"}
	 *         当传入的参数useranduser_id为null或""时,返回值为{"result" = "null"} 当程序报错时
	 *         返回值为{"result" = "error"}
	 * @throws Exception
	 */
	...
	public Object deleteRelationUser(@RequestBody String u) {
		...
		JSONObject jasonObject = JSONObject.fromObject(u);
		try {
			if (jasonObject.get("useranduser_id") != null && !"".equals(jasonObject.get("useranduser_id"))
					&& !"null".equals(jasonObject.get("useranduser_id"))) {
	...
	/**
	 * 根据userId 添加关联用户 url:http://localhost:8080/ihealth/rest/saveUserAndUser
	 * type:post 传入参数
	 * 
	 * @param {
	 *            "userId":"当前登录的用户ID(主ID)", "user_Id":"关联用户的ID",
	 *            "connection":"用户关系" }
	 * @return 当传入的参数为null 返回{"result":"error"}
	 *         当新增关联关系成功时！返回{"result":"success"} 如果关系已存在
	 *         返回：{"result":"existence"} 程序出错 返回{"result":"no"}
	 * @throws Exception
	 */
	...
	public Object saveUserAndUser(@RequestBody String u) throws Exception {
		...
		try {
			if (jasonObject.get("connection") != null && !"".equals(jasonObject.get("connection"))
					&& !"null".equals(jasonObject.get("connection"))) {
				connection = jasonObject.getString("connection").trim();
			}

			if (jasonObject.get("userId") != null && !"".equals(jasonObject.get("userId"))
					&& !"null".equals(jasonObject.get("userId")) && jasonObject.get("user_Id") != null
					&& !"".equals(jasonObject.get("user_Id")) && !"null".equals(jasonObject.get("user_Id"))) {
				userId = jasonObject.getString("userId").trim();
	...
	/**
	 * 根据userId 查询用户信息完整度
	 * url:http://localhost:8080/ihealth/rest/findMessageIntegrity type:post
	 * 
	 * @param {"userId":"当前登录的用户ID(主ID)"}
	 * @return 用户ID不为空 返回 { "result":"success", "data": { "tagNumber":标签信息完整度,
	 *         "connectionNumber":关心的人完整度, "USER_ID":"用户ID",
	 *         "diseaseNumber":疾病信息完整度, "userNumber":用户信息完整度 } } 传入的用户ID为空
	 *         返回{"result":"null"} 程序出错 返回{"result":"error"}
	 * 
	 */
	...
	public Object findArticleByParams(@RequestBody String u) throws Exception {
		...
		Map<Object, Object> map = new HashMap<Object, Object>();
		String msg = null, userId = null, tags = null, personalDiseases = null, familyDiseases = null,
				concernDiseases = null;
		PageData pd = new PageData();
		pd = this.getPageData();
		...
		// 判断userId 是否为空
		if (jasonObject.get("userId") != null && jasonObject.get("userId") != "null"
				&& jasonObject.get("userId") != "") {
