...
/**
 * 类名称：RestfullController 提供微信端网页访问的接口 创建人：shouxin 更新时间：2015年11月3日
 * 
 * @version
 */
...
public class RestfullController extends BaseController {
	...
	/**
	 * 用户注册，通过手机号码 url : rest/register type:post
	 * 
	 * @param {phone:"xx",openid:"xxx"}
	 * @return 当手号码不存在时，执行新增用户并返回用户信息{"result": "success","data": {"OPENID":
	 *         "OPENID","PHONE": "电话","USER_ID": "USER_ID","ROLE_ID": "用户权限"}}
	 *         当手机号码存在，返回 { "result": "existence", "data": { "NUMBER": "编号",
	...
	/**
	 * 根据ID更新用户信息 URL：rest/updateUser type:post ID不为空 返回结果，和用户信息： { "result":
	 * "suceess", "data": { "NUMBER": "编号", "RIGHTS": "权限", "IP": "IP地址",
	 * "PHONE": "电话", "ALIAS": "昵称", "SEX": "性别", "USER_ID": "ID",
	 * "MARRIAGESTATUS": "婚姻状况", "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址",
	 * "HEIGHT": 身高, "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业",
	 * "STATUS": "0", "OPENID": "openId", "PASSWORD": "密码", "BZ": "333",
	 * "USERNAME": "用户名", "DEGREE": "学历", "LIVEPLACE": "常住地", "AVATAR": "头像地址",
	 * "WEIGHT": 体重, "BIRTHDAY": "生日" } } ID为空： {"result":"error"}
	 * 
	 * @param {"userId":"ID","name":"姓名","sex":"性别","marriageStatus":"婚姻",
	 *            "birthday":"生日","height":"身高","weight":"体重","birthPlace":
	 *            "出生地",
	 *            "livePlace":"常住地","career":"职业","degree":"学历",}
	 * @return Object json
	 * @throws Exception
	 */
	...
	// 通过用户ID获取体检套餐信息的接口
	/**
	 * url:http://localhost:8080/ihealth/rest/findCheckPackage type:post
	 * 通过用户ID获取体检套餐信息
	 * 
	 * @param {"userId":"用户ID"}
	 * @return 当用户ID不为空时，返回： { "result": "success", "data": { "REVISION": "版本",
	 *         "STATUS": "状态", "GENERATEDTIME": 该记录生成时间, "EFFECTIVEFROM":
	 *         开始生效时间, "EXPIREON": 失效时间, "WORKER": "生成该记录的标记",
	 *         "CHECKUPPACKAGE_ID": "1001", "SYSFLAG": "系统标记" } }
	 * 
	 *         当用户ID为空时，返回：{ "result": "error"} 当根据userID查询出的数据为null时 返回{
	 *         "result": "no"}
	 * @throws Exception
	 */
	...
	/**
	 * url:http://localhost:8080/ihealth/rest/editCheckItem type:post
	 * 根据体检项目ID修改体检项目的状态信息
	 * 
	...
	/**
	 * 根据userID 获取体检项目 本机url：http://localhost:8080/ihealth/rest/findCheckItems
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 * @return 当userID不为空，并且数据库中存在这个ID、返回以下数据，需要注意的是，一个用户有多个体检项目，需要循环取值 {
	 *         "result": "success", "data": [ { "REVISION": 版本, "STATUS":
	 *         "状态，包括：已选中，已删除", "DESCRIPTION": "详细描述", "GENERATEDTIME": 该记录生成时间,
	 *         "FREQUENCY": "每年一次", "ORIGINATE": "指南来源", "WORKER": "用于产生该记录的标记",
	 *         "SUBGROUP": "检查项目分组", "SYSFLAG": "系统标记", "NAME": "检查项目名称",
	 *         "FEATURES": "检查频率，是文字描述", "CHECKUPITEM_ID": "ID" } ] }
	 *         当用户ID为空时返回一下数据{"result": "error"} 当根据userID查询出的数据为null时 返回{
	 *         "result": "no"}
	 * @throws Exception
	 */
	...
	/**
	 * 通过用户ID获取用户信息 本机地址url:http://localhost:8080/ihealth/rest/findUserById
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 ...
	 *         "头像地址", "WEIGHT": 体重, "BIRTHDAY": "生日" } }
	 *         当userID为空时：返回的参数为：{"result": "error"} 当根据userID查询出的数据为null时 返回{
	 *         "result": "no"}
	 * @throws Exception
	 */
	 ...
	 * @return 当获取openId成功！返回值为： { "result": "success", "data": { "NUMBER":
	 *         "编号", "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话", "ALIAS": "昵称",
	 *         "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS": "婚姻状况",
	 *         "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业", "STATUS":
	 *         "用户状态", "OPENID": "openId", "PASSWORD": "密码", "BZ": "333",
	...
	// 根据用户ID 获取文章信息
	/**
	 * 根据用户ID 获取文章信息 url:http://localhost:8080/ihealth/rest/findArticleByUserId
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 * @return 用户ID不为空 返回： { "result": "success", "data": [ { "LOGOURL": 照片url
	 *         "PUBLISHTIME": 发布时间 "SUMMARY": "摘要", "CREATEBY": "创建记录员工id",
	 *         "TITLE": "标题", "ARTICLE_ID": "ID", "AUTHOR": "作者", "CREATEON":
	 *         创建记录时间 "URL": 内容url } ] }
	 * 
	 *         当userID为空： 返回{ "result": "error"} 当根据userID查询出的数据为null时 返回{
	 *         "result": "no"}
	 * @throws Exception
	 */
	...
	/**
	 * 根据用户ID 获取标签信息 url:http://localhost:8080/ihealth/rest/findTagByUserId
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 * @return 用户ID不为空 返回： { "result": "success", "data": [ { "CREATEBY":
	 *         "创建用户ID", "TAG_ID": "ID", "CREATEON": "创建时间", "EXPRESSION":
	 *         "表达式", "NAME": "标签名" } ] } 当userID为空： 返回{ "result": "error"}
	...
	/**
	 * 根据用户ID 获取疾病信息 url:http://localhost:8080/ihealth/rest/findDiseaseById
	 * type:post
	 * 
	 * @param {"userId":"1"}
	 * @return 用户ID不为空 返回： { "result": "success", "data": [ { "CREATEBY":
	 *         "创建记录员工id", "DESCRIPTION": "描述", "ISINHERITABLE": 是否遗传倾向,
	 *         "DISEASECATEGORY_ID": "疾病分类ID", "ISHIGHINCIDENCE": 是否高发,
	 *         "DISEASE_ID": "ID", "CREATEON": "创建记录时间", "NAME": "名称" } ] }
	 *         当userID为空： 返回{ "result": "error"} 当根据userID查询出的数据为null时 返回{
	 *         "result": "no"}
	 * @throws Exception
	 */
	 ...
	 *         当根据userID查询出的数据为null时 返回{ "result": "no"}
	 *         当查询出数据时、返回一条或多条数据，取值需要循环： { "result": "success", "data": [ {
	 *         "NUMBER": "编号", "RIGHTS": "权限", "IP": "IP地址", "PHONE": "电话",
	 *         "ALIAS": "昵称", "SEX": "性别", "USER_ID": "ID", "MARRIAGESTATUS":
	 *         "婚姻状况", "LAST_LOGIN": "最后登录时间", "EMAIL": "邮箱地址", "HEIGHT": 身高,
	 *         "BIRTHPLACE": "出生地", "NAME": "姓名", "CAREER": "职业", "STATUS":
	 *         "用户状态", "OPENID": "openId", "PASSWORD": "密码", "BZ": "333",
	 *         "USERNAME": "用户名", "DEGREE": "学历", "LIVEPLACE": "常住地", "AVATAR":
	 *         "头像地址", "WEIGHT": 体重, "BIRTHDAY": "生日" } ] }
	 ...
	 * @return 删除成功返回:{"result": "success"} 当传入的参数useranduser_id
	 *         为null或""时,返回值为{"result" = "null"} 当程序报错时 返回值为{"result" = "error"}
	 */
	...
	/**
	 * 根据userId 添加关联用户 url:http://localhost:8080/ihealth/rest/saveRelationUser
	 * 
	 * @param {"userId":"当前登录的用户ID(主ID)","name":"姓名","sex":"性别","marriageStatus"
	 *            :"婚姻","birthday":"生日","height":"身高","weight":"体重","birthPlace"
	 *            :"出生地",
	 *            "livePlace":"常住地","career":"职业","degree":"学历",}
	 * @return 当新增用户信息失败时！跟新增关联用户关系失败时！返回{"result":"error"}
	...
	public Object saveRelationUser(@RequestBody String u) {
		/*
		 * 分析：当用户点击新增关联用户按钮时：获取当前登录的用户ID userId 跳转页面到新增 (执行新增用户的操作) 同时保存 生成的 ID
